...
The Ivory monad tracks effects (the \cd{eff} type parameter); see Section~\ref{sec:ivory-monad}.
...
current region to the monad, and the allocation function \cd{local} returns a
reference in the current region.  For example, the following constructs a zero
initialized reference to an integer; the \cd{ival} constructs an initializer
from a value:
\begin{code}
