...
When programming in C, these values are typically accessed by defining
a set of integer constants and using bit operations to shift and
mask the correct bits into place with little to no type checking.

\james{TODO: One more paragraph about how we do better in Ivory using a bit
data system based on the design described in Iavor's paper.}
...
\james{Show how the accessors are type-safe so we can't try to set
fields in the wrong register to tie this back to the intro.}
...
\james{Show the C code this expands into for efficiency and atomicity?}
