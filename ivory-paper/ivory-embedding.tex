...
introduce \cd{arrayMap}, whose signature is shown in Figure~\ref{array-support-functions}.  The intuition for the \cd{arrayMap} function is
...
\end{figure}
\eric{When I read this, being somewhat out of the loop, I was certain that
  arrayMap was missing the array parameter. But it actually isn't since the
  bound is inferred by the usage inside the loop body. You should be more
  explicit about not needing the array as a parameter because it's both odd and
  very neat!}
...
\eric{This is not actual paper text!\\
...
\paragraph{Defining Bit Data Types}
...
define accessors for the fields of a \cd{CtrlReg},
and have the types:
