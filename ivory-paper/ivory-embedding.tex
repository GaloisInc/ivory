...
heavily inspired by the work of Diatchki and Jones~\cite{memareas}.  The type
parameter on the \cd{Area} kind is present so that when giving kind-signatures,
we can fix the kind of stored-values as being star (\cd{*})-kinded. \lee{I think this is a detail
  that I'd elide} As Haskell currently lacks a
...
This technique was described by Magalh\~{a}es~\cite{jpm:trkgp:12}.
...
(2) we cannot statically know the lifetime of a reference when it can be
