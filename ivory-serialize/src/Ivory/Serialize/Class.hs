...
class SerializableRef t where
  ...
  default packRef :: (Serializable s, t ~ Stored s) => Ref s1 (CArray (Stored Uint8)) -> Uint32 -> ConstRef s2 t -> Ivory eff ()
  packRef into at from = deref from >>= pack into at

  -- Given a ref and a byte offset into an array, unpack the value of t starting
  -- at the offset.
  unpackRef :: ConstRef s1 (CArray (Stored Uint8)) -> Uint32 -> Ref s2 t -> Ivory eff ()
  default unpackRef :: (IvoryStore s, Serializable s, t ~ Stored s) => ConstRef s1 (CArray (Stored Uint8)) -> Uint32 -> Ref s2 t -> Ivory eff ()
  unpackRef from at to = unpack from at >>= store to

instance (ANat len, IvoryArea a, SerializableRef a) => SerializableRef (Array len a) where
